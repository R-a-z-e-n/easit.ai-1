...
const getApiKey = () => {
  try {
    const key = process.env.API_KEY || "AIzaSyB2B132gxcIRhgG0nii6dryOlbJ4La4Pkg";
...
export const useGeminiLive = () => {
  ...
  // Refactor: Use type inference for Set to avoid nested generic syntax <Set<T>> which can confuse JSX parsers.
  ...
  const startSession = useCallback(async ({ onTurnComplete, systemInstruction, apiKey }: StartSessionOptions) => {
    ...
    try {
      ...
      sessionPromiseRef.current = ai.live.connect({
        ...
        callbacks: {
          ...
          onerror: (e: ErrorEvent) => {
            ...
            const errorMessage = e instanceof Error ? e.message : 'Connection error. Please check your API key.';
            setError(errorMessage || 'Connection error. Please try again.');
      ...
      setError(errorMsg || 'Could not access microphone. Please check permissions.');
      setStatus(GeminiLiveStatus.ERROR);
      cleanup();
    }
  }, [cleanup, stopSession]);
